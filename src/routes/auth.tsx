...
export const Route = createFileRoute("/auth")({
  ...
  head: () => ({
    meta: [
      { title: "Sign in — Sammy Store Logs" },
      { name: "description", content: "Sign in or create your Sammy Store Logs account." },
    ],
  }),
  component: AuthPage,
});
...
function AuthPage() {
  ...
  useEffect(() => {
    if (!loading && user) {
      navigate({ to: (search.redirect as "/" | undefined) ?? "/dashboard", replace: true });
    }
  ...
  return (
    <div className="min-h-[calc(100vh-200px)] flex items-center justify-center px-4 py-12 bg-background">
      <div className="w-full max-w-md bg-card border border-border rounded-2xl shadow-sm p-6 sm:p-8">
        ...
        <Tabs defaultValue={search.mode === "signup" ? "signup" : "login"} className="w-full">
